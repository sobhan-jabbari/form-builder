...
public class FieldDatePicker extends FormLayout implements OnValueChangedListener<PersianDate> {
  ...
  private FieldDatePicker.OnDatePickedListener onDatePickedListener;

  public FieldDatePicker(@NonNull Context context) {
    this(context, null, 0);
  }

  public FieldDatePicker(@NonNull Context context, @Nullable AttributeSet attrs) {
    this(context, attrs, 0);
  }

  public FieldDatePicker(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
  ...
  private void prepareDatePicker() {
    ...
    if (this.minYear == 0) {
      this.minYear = persianDate.getYear() - 150;
    }

    if (this.maxYear == 0) {
      this.maxYear = persianDate.getYear() + 150;
    }

    if (this.getContext() instanceof AppCompatActivity) {
      AppCompatActivity activity = (AppCompatActivity) this.getContext();
      DialogDatePicker.show(activity.getSupportFragmentManager(), this.minYear, this.maxYear, this.date == 0L ? null : persianDate, this.dialogBackgroundColor, this);
    }
  ...
  public void setOnDatePickedListener(FieldDatePicker.OnDatePickedListener onDatePickedListener) {
  ...
  @Override
  public void onValueChanged(PersianDate persianDate) {
    this.setError(null);
    this.setValue("" + persianDate.toCalendarTime() / 1000L);
    if (this.onDatePickedListener != null) {
      this.onDatePickedListener.onDatePicked(persianDate);
    }

  }

  public interface OnDatePickedListener {
    void onDatePicked(PersianDate var1);
  }
}
